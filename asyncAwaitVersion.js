...
async function doSummerChores(pickedName) {
    try {
        await mowYard(pickedName)
        await weedEat(pickedName)
        await trimHedges(pickedName)
        await collectWood(pickedName)
        await waterGarden(pickedName)
    } catch (error) {
        console.log(error);
    }
}
